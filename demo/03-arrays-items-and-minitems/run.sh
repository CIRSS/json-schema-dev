...
Demo 02 described objects with properties, which gives one schema per
named member. Arrays have no member names -- their elements are known
only by position -- so the keyword that describes them, items, takes a
simpler form. It is worth recalling first that JSON puts no
restriction on what an array holds: [1, "x", null, {}] is a legal
array, its elements of four different kinds. Uniformity, where a
schema wants it, is a constraint the schema must state. The value of items is one schema, and that one schema
applies to every element: an array satisfies items only if each of its
elements satisfies the schema. So {"type": "array", "items": {"type":
"number"}} says: the instance is an array, and each of its elements is
a number.
