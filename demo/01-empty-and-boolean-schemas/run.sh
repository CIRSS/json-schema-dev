...
A schema is itself JSON -- a single JSON document, and it can be
a value of only two of JSON's six kinds: boolean, or object --
which, despite its name, is just a string-keyed map, written
{...}. Both boolean values are schemas: true is the schema that
accepts every instance, false the schema that rejects every
instance. The map can be empty: {} is a schema, declares no
constraints, and so behaves exactly like true. Some objects are
valid schemas and some are not; the last cells below show the
difference.
...
schema that declares a different version (demo 22 shows this).
END_DOC
...
look for members beyond the standard's own (demo 19 uses this to give
...
validators refuse to proceed rather than render a verdict.

The document that decides is itself a schema: what counts as a
schema is specified the way anything in JSON is specified -- by a
schema. That schema-for-schemas, the meta-schema, is a JSON
document that both validators ship as data, and almost all of its
content states which member names are keywords and the form each
keyword's value must take. Checking an offered schema is ordinary
validation -- the meta-schema as the schema, the offered document
as the instance -- and this document's "type" member, paired with
the value 5, failed to be meaningful.
