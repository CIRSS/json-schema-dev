...
satisfies such a type if it is a value of any type in the list -- the
...
properties is the keyword that describes the members of an object. Its
value is itself an object: each key is a member name, and each value
is a schema that the member of that name must satisfy, if the member
is present. So {"properties": {"a": {"type": "number"}}} says: if the
instance has a member named a, that member's value must be a number.
Because each value in a properties object is a complete schema, the
same keywords used at the top level are used again inside it -- here,
type -- and this nesting is how schemas describe structure at any
depth. Demo 08 takes up what it means for a schema to sit inside a
schema.

Nearly every schema in practice is a type of object together with a
...
properties is conditional. Each entry says "if a member of this name is
present, here is what its value must satisfy," and nothing more. It
does not require the member to be present -- that is a separate
keyword's job (required, demo 04) -- and it does not forbid members it
does not list; an instance may carry any number of members the schema
never mentions. Forbidding unlisted members takes a keyword of its
own, taken up in demo 13.
