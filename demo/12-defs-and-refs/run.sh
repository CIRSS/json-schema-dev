...
instance unless referenced somewhere else, yet is required to be a
valid schema. The key an entry is filed under -- Hex below -- has no
meaning of its own; it simply becomes part of the path a "$ref" walks.

Together the two play the part that type definitions play in a
programming language. The value of each "$ref" here is a path into the
schema document: # is the document itself, and /$defs/Hex walks from
there to the definition. (A "$ref" can also name a schema in other
ways; demo 13 shows them.)

The schema below defines Hex once -- a lowercase hexadecimal string,
anchored with ^ and $ and ending with the $(?!\n) guard against a
trailing newline, both from demo 07 -- and references it from two
properties. Lowercase only is deliberate: a parser reading hex as a
number accepts either case, but a checksum is compared as a string,
where DEADC0DE and deadc0de are different -- so the definition pins one
canonical case, as git does for its object ids. Editing the definition
changes what both properties enforce.
...
sits at $defs/Hex -- a one-character typo. The two validators notice at
different moments. Ajv resolves every reference while compiling the
schema, so it
