...
| `05-null-and-applicability` | `minLength`, `enum` vs `minimum`, `null` in a `type` list | keywords are type-scoped and pass silently when inapplicable; widening a type to admit `null` exempts it from the field's own constraints |
