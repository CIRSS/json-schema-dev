...
| `08-schemas-in-schema-position` | `additionalProperties` | the slot as a schema, not a flag; a non-boolean schema in it |
| `09-annotated-constants` | `oneOf`, `const` vs `enum` | alternation over documented constants; same predicate as flat `enum`; per-value prose; noisier errors |
| `10-tagged-unions` | `if`/`then`/`else` | a discriminator selecting constraints; `if` must `require` its discriminator or `{}` falls into `then` |
| `11-defs-and-refs` | `$defs`, `$ref`, `allOf` | a definition written once, enforced everywhere; composition by intersection; a typo'd `$ref` failing at compile time in Ajv, on first use in `jsonschema` |
| `12-identifiers-and-refs` | `$id`, `$anchor`, `$ref` | `$id` inert until a `$ref` resolves against it; resolution landing away from the intended `$def` |
| `13-unevaluated-properties` | `unevaluatedProperties: false` over `allOf` | closure that sees through `allOf`; `additionalProperties: false` that cannot |
| `14-closure-scope` | `$ref`, `$defs`, `unevaluatedProperties` vs `additionalProperties` | closure reaching sideways through `$ref` but not downward into a nested object |
| `15-multi-file-extension` | `--ref`, `$id`-based `$ref`, `unevaluatedProperties` over both | a standalone base schema; an overlay file referencing it, adding constraints, closing over base and overlay properties together |
| `16-custom-error-messages` | `errorMessage` (nonstandard) | message text left to validators by the standard; authored messages in the schema, honored by both wrappers; `${0}` quoting the failing value inside arrays |
| `17-non-json-instances` | the wrappers' exit-code contract | non-JSON instance as verdict (exit 1), missing file as error (exit 2); instance facts are verdicts, everything else errors; parser detail diverging like all message prose |
| `18-duplicate-member-names` | `--reject-duplicate-members` | last-value-wins parsing, invisible to any schema; a `const` on the last value passing; the flag as parse-time verdict, identical across wrappers |
| `19-declared-versions` | `$schema`, `--ignore-declared-version` | non-2020-12 declarations refused, not reinterpreted; draft-07 `definitions` as ignored unknown members; the override as explicit choice |
