...
| # | Construct | Shows |
| --- | --- | --- |
| `01-empty-and-boolean-schemas` | `{}`, `true`, `false` | a schema is itself JSON, admitted by the meta-schema: booleans and (not all) objects — accept-all and reject-all at the schema floor, the instance floor beside it (any of JSON's six value kinds is a complete document), and the boundary at the end: unknown members ignored, one malformed keyword value and the object is no schema at all |
| `02-types-and-properties` | `type`, `properties` | `type` names the kind of the whole instance — a bare string is a valid document for `{"type": "string"}` — or lists several kinds, any of which satisfies it; `properties` maps member names to schemas, neither requiring the members it names nor forbidding the ones it doesn't |
| `03-arrays-items-and-minitems` | `items`, `minItems` | `items` holds one schema every element must satisfy — the array counterpart of `properties`, and the same schema-inside-schema nesting; it says nothing about how many elements, so `[]` satisfies any `items` schema; `minItems: 1` is what rules the empty array out, and it is a floor, not a count |
| `04-empty-vs-nonempty-object` | `required` | `properties` alone accepts `{}`; `required` — a list of member names — is what rejects it; and `required` asks about presence, not content: `{"id": null}` satisfies `required` alone and is rejected only by a value constraint such as a typed `properties` entry |
| `05-null-and-applicability` | `minLength`, `minimum`, `enum`, `null` in a `type` list | most keywords are scoped to one kind and pass silently on any other — silent means satisfied; permitting `null` (by naming it in `type`; there is no nullable switch) exempts it from every string constraint in the schema; value-comparison keywords like `enum` are the exception and reject `null` |
...
| `19-declared-versions` | `$schema`, `--ignore-declared-version` | a schema declaring any version other than 2020-12 is refused rather than silently reinterpreted (a draft-07 author's `definitions` would become ignored unknown members); the override flag discards the declaration and validates as 2020-12 as an explicit choice |
